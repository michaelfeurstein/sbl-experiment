...
File: ***02_accuracy_features_flat_prepared.csv*** - contains all detailed measurements of the feature accuracy (1st part of the calculation for accuracy). Note that an additional subset of this dataset ***data_assessed/accuracy_feature_detail_F2e.csv*** contains the assessment of the reason for a specific error F2e: timestamp assignment to video

```{r}
# read from prepared csv
f_data <- read.csv("data_prepared/02_accuracy_features_flat_prepared.csv")
f2e_details <- read.csv("data_assessed/accuracy_feature_detail_F2e.csv")
```
...
##### F2e assignment to video

The following plot is used in Section 6.5 Prototype Improvements to showcase the characteristics/reason for error of assigning timestamps to a video: "Why did this error happen?"

```{r}
longer_f2e_details <- pivot_longer(f2e_details, cols = reason.code)

# expand = expansion(mult = c(0.02, .2)): left 2% right 20%
f2e_details_plot <- ggplot(data = longer_f2e_details, aes(x =fct_rev(fct_infreq(value)))) + 
  geom_bar(fill = "grey", color = "black", width = 0.85) +
  coord_flip() +
  theme_classic2() +
  theme(legend.position="none",
        plot.title = element_text(size = 18, colour = "black", face = "bold", hjust = -0.05),
        axis.title.x = element_blank(),
        axis.title.y = element_blank(),
        axis.text = element_text(size = 16, colour = "black"),
        plot.margin = margin(0.3,0.3,0.3,0.3, "cm")) + 
  ...
  labs(title = "Timestamp assignment error", subtitle = "Why did the timestamp assignment fail?")

# save
ggsave(file="plots/accuracy_f2e_reason.svg", plot=f2e_details_plot, width = 2100, height = 1400, units = "px", dpi = 300)
ggsave(file="plots/accuracy_f2e_reason.eps", plot=f2e_details_plot, width = 2100, height = 1400, units = "px", dpi = 300)
ggsave(file="plots/accuracy_f2e_reason.png", plot=f2e_details_plot, width = 2100, height = 1400, units = "px", dpi = 300)
#print
f2e_details_plot
```
