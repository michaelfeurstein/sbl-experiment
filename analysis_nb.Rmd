...
model_seq<-lmer(duration.log~notation.r+period+sequence+(1|subject),data=df)
summary(model_seq)
```

From the above we can see that there is a period effect. Note that H0 refers to the preiod effect.
...
model_int<-lmer(duration.log~notation.r*period+(1|subject),data=df)
summary(model_int)
```
...
We always run both tests. For the variable efficiency (duration) data is normal, therefore we highlight result from paired t-test.
...
```{r}
summary(glht(model_seq, linfct=mcp(notation.r="Tukey")))
...
ggplot(data=duration_original_CI) +
  ...
  coord_flip() +
  ylab('Differences in mean levels of duration') +
  xlab('CNL-KV') + 
  labs(title = "95% family-wise confidence level", subtitle = "Tukey HSD") + theme_bw() + theme(legend.position="none",
...
# CONTINUE HERE: create table for insertion in thesis with all the values from above and the tukey plot above or below the table.!!!
